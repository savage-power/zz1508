...
- (void)setup{
    ...
    if (WMMenuItemWidth *self.titles.count < kScreenSize.width) {
        self.menuItemWidth=kScreenSize.width/self.titles.count;
    }else{
...
- (void)calculateSize{
    ...
    if (self.navigationController) {
        viewHeight = kScreenSize.height - self.menuHeight - 64;
    }else{
        viewHeight = kScreenSize.height - self.menuHeight - 0;
    }
    viewWidth = kScreenSize.width;
}
...
- (void)addMenuViewWith:(CGFloat)menuY{
    ...
    CGRect frame = CGRectMake(0, 0, kScreenSize.width, self.menuHeight);
